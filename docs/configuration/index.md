...
SliDesk uses a `slidesk.toml` file for presentation-level configuration.

Values in this file are accessible in three ways:
- **Plugins** via `window.slidesk.env.KEY`
- **Slide content** via the `++KEY++` syntax
- **SliDesk itself** via the `[slidesk]` section keywords
...
Enable HTTPS. Requires `KEY` and `CERT` to be set.
...
If your presentation is designed at 1920px wide and you insert a 960px-wide image, it will be displayed at 50% width.

## TITLE

The content of the HTML `<title>` tag.
...
Slide transition animation duration in milliseconds. Default: `300`.
...
Enable or disable the file watcher. When enabled (default), editing a file in your talk directory triggers an automatic browser refresh.
...
Any key-value pair under `[slidesk]` is accessible as an environment variable:
